        ...
        <div class="card">
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between">
                    ...
                    <a class="btn btn-primary" href="{{ route('section.add') }}">إضافة</a>
                </div>
                <p class="tx-19 tx-gray-500 mb-2">يمكنك إضافة و تعديل الاصناف من هنا .</p>
            </div>
            <div class="card-body">
                    ...
                    <table class="table text-md-nowrap" id="example2">
                        <thead>
                            <tr>
                                ...
                                <th class="col-2 smaller-section-name border-bottom-0">إسم الصنف</th>
                                <th class="col-2 border-bottom-0">حالة الصنف</th>
                                <th class="col-2 border-bottom-0">تعديل</th>
                        ...
                        <tbody>
                            @foreach ($sections as $section)
                                <tr>
                                    ...
                                    <td>{{ $section->name }}</td>
                                    <td>
                                        <form method="POST" action="{{ route('section.status') }}">
                                            @csrf
                                            <input type="text" name="id" value="{{  $section->id }}" hidden>
                                            <button class="btn {{ $section->status == 1 ? 'btn-success' : 'btn-danger'  }} " href="">
                                                {{ $section->status == 1 ? 'Active' : 'Inactive'  }}
                                            </button>
                                        </form>
                                    </td>
                                    <td>
                                        <a class="btn btn-primary" href="{{ route('section.show', $section->id) }}">
